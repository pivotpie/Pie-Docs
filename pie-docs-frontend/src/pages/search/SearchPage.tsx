...
import { RAGQueryInterface } from '@/components/search/RAGQueryInterface';
import { RAGSearchResults } from '@/components/search/RAGSearchResults';
import { ChunkSearchResults } from '@/components/search/ChunkSearchResults';
import { searchService } from '@/services/api/searchService';
// Note: These components will be implemented in future iterations
// import { IntelligentAnswerGenerator } from '@/components/search/IntelligentAnswerGenerator';
// import { DocumentRelationshipMapper } from '@/components/search/DocumentRelationshipMapper';
// import { SearchAnalyticsOptimization } from '@/components/search/SearchAnalyticsOptimization';
...
export const SearchPage: React.FC = () => {
  ...
  const handleAdvancedQuery = useCallback((advancedQuery: any) => {
    handleSearch(advancedQuery.naturalLanguage, {});
  }, [handleSearch]);
  ...
  const handleRAGQuery = useCallback(async (query: string) => {
    ...
    try {
      const response = await searchService.ragQuery(query, 5);
      setRagState({
        ...response,
        isLoading: false,
      });
  ...
  // Chunk Search Handler
  const handleChunkSearch = useCallback(async (query: string) => {
    setChunkState({
      query,
      chunks: [],
      isLoading: true,
          ...
          <div className="space-y-6">
            <div className="glass-card p-6">
              ...
              <AdvancedSearchBuilder
                onQueryBuild={handleAdvancedQuery}
                onPreview={(query) => handleSearch(query)}
              />
            </div>
          ...
          <div className="space-y-6">
            <div className="glass-card p-6">
              ...
              <p className="text-white/60 text-sm mb-6">
                Ask questions in natural language and get AI-generated answers with source attribution
              </p>
              <RAGQueryInterface
            ...
            {/* RAG Results */}
            {ragState && !ragState.isLoading && ragState.answer && (
              <RAGSearchResults
                query={ragState.query}
                answer={ragState.answer}
                confidence={ragState.confidence}
                relevant_chunks={ragState.relevant_chunks}
                sources={ragState.sources}
                timeTaken={ragState.timeTaken}
              />
            )}
            ...
            {/* Alternative: Chunk-Level Search */}
            <div className="glass-card p-6">
              <h3 className={`text-lg font-semibold mb-4 ${theme === 'dark' ? 'text-white' : 'text-white/90'}`}>
                Or Search by Chunks
              </h3>
              <p className="text-white/60 text-sm mb-4">
                Find specific sections within documents with similarity scores
              </p>
              <SearchInput
                value={chunkState?.query || ''}
                onSearch={handleChunkSearch}
                isLoading={chunkState?.isLoading}
                placeholder="Search for specific content chunks..."
              />
            </div>

            {/* Chunk Search Results */}
            {chunkState && chunkState.chunks.length > 0 && (
              <ChunkSearchResults
                chunks={chunkState.chunks}
                query={chunkState.query}
                isLoading={chunkState.isLoading}
                onDocumentClick={(docId) => handleDocumentPreview(docId)}
              />
            )}
          </div>
        );
