...
export const MetadataSchemaDesigner: React.FC<MetadataSchemaDesignerProps> = ({
  ...
  const renderFieldEditor = (field: MetadataField) => {
    ...
    return (
      <div key={field.id} className="glass-panel rounded-lg p-4 border border-white/20">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-3">
            {fieldType && <fieldType.icon className="w-5 h-5 text-white/60" />}
            <input
              type="text"
              value={field.label}
              onChange={(e) => handleUpdateField(field.id, { label: e.target.value })}
              className={`font-medium ${theme === 'dark' ? 'text-white' : 'text-white/90'} bg-transparent border-none focus:outline-none focus:ring-0 p-0`}
            />
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`p-1 ${theme === 'dark' ? 'text-white/60' : 'text-white/60'} hover:text-white/80 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 glass-panel border border-white/20 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400 ${theme === 'dark' ? 'text-white' : 'text-white'} placeholder-white/50`}
          ...
          <div>
            ...
            <select
              ...
              className={`w-full px-3 py-2 glass-panel border border-white/20 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400 ${theme === 'dark' ? 'text-white' : 'text-white'}`}
              disabled={field.isSystem}
            >
              {fieldTypes.map(type => (
          ...
          <div className="flex items-center space-x-4">
            <label className="flex items-center space-x-2">
              <input
                ...
                disabled={field.isSystem}
              />
              <span className={`text-sm ${theme === 'dark' ? 'text-white/80' : 'text-white/80'}`}>Required</span>
            </label>
          </div>
        ...
        >
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1">
              <div className="flex items-center space-x-3 mb-2">
                ...
                <span className={`text-sm ${theme === 'dark' ? 'text-white/60' : 'text-white/60'}`}>v{schema.version}</span>
                {schema.isActive && (
                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium glass-panel border border-green-400/30 text-green-300">
                    Active
                  </span>
                )}
              </div>
              ...
              {schema.description && (
                <p className={`${theme === 'dark' ? 'text-white/80' : 'text-white/80'} mb-2`}>{schema.description}</p>
              )}

              <div className={`flex items-center space-x-4 text-sm ${theme === 'dark' ? 'text-white/60' : 'text-white/60'}`}>
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className={`p-2 ${theme === 'dark' ? 'text-white/60' : 'text-white/60'} hover:text-blue-400 hover:scale-110 transition-all duration-200`}
              ...
              <button
                ...
                className={`p-2 ${theme === 'dark' ? 'text-white/60' : 'text-white/60'} hover:text-green-400 hover:scale-110 transition-all duration-200`}
                title="Edit schema"
              >
              ...
              {schema.usageStats.documentsCount === 0 && (
                <button
                  ...
                  className={`p-2 ${theme === 'dark' ? 'text-white/60' : 'text-white/60'} hover:text-red-400 hover:scale-110 transition-all duration-200`}
                  title="Delete schema"
                >
                  <TrashIcon className="w-4 h-4" />
                </button>
          ...
          <div className="flex flex-wrap gap-1">
            {schema.documentTypes.map(type => (
              <span
                key={type}
                className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium glass-panel border border-white/20 text-white/80"
              >
                {type.toUpperCase()}
              </span>
            ))}
    ...
    return (
      <div className="space-y-4">
        ...
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="glass-panel border border-white/20 rounded-lg p-4">
      ...
      <div className="tab-content">
        ...
        {activeTab === 'designer' && editingSchema && (
          <div className="space-y-6">
            {/* Schema Properties */}
            <div className="glass-card rounded-lg border border-white/20 p-6">
              <h3 className={`text-lg font-semibold ${theme === 'dark' ? 'text-white' : 'text-white/90'} mb-4`}>
                Schema Properties
              </h3>
            ...
            <div className="glass-card rounded-lg border border-white/20 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="flex items-center px-3 py-1.5 btn-glass rounded-md hover:scale-105 transition-all duration-300"
                >
              ...
              <div className="space-y-4">
                ...
                {editingSchema.fields.length === 0 && (
                  <div className={`text-center py-8 ${theme === 'dark' ? 'text-white/60' : 'text-white/60'}`}>
            ...
            <div className="flex justify-end space-x-3">
              <button
                ...
                className={`px-4 py-2 ${theme === 'dark' ? 'text-white/80' : 'text-white/80'} glass-panel border border-white/20 hover:bg-white/10 rounded-md hover:scale-105 transition-all duration-300`}
              >
                Cancel
              </button>
              <button
                onClick={handleSaveSchema}
                className="px-4 py-2 btn-glass rounded-md hover:scale-105 transition-all duration-300"
              >
                Save Schema
              </button>
            </div>
                ...
                .map(field => {
                  ...
                  return (
                    <div key={field.id} className="space-y-1">
                      <label className={`block text-sm font-medium ${theme === 'dark' ? 'text-white/80' : 'text-white/80'}`}>
                        {field.label}
                        {field.required && <span className="text-red-500 ml-1">*</span>}
                      </label>

                      {field.description && (
                        <p className={`text-xs ${theme === 'dark' ? 'text-white/60' : 'text-white/60'}`}>{field.description}</p>
                      )}
