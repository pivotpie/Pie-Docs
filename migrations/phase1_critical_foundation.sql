...
CREATE INDEX idx_mfa_codes_session ON mfa_codes(session_id);
CREATE INDEX idx_mfa_codes_expires ON mfa_codes(expires_at) WHERE verified = false;
...
-- 5.3 Add check constraint (drop if exists first)
...
CREATE TABLE IF NOT EXISTS document_permissions (
    ...
    CONSTRAINT unique_doc_perms_user UNIQUE(document_id, user_id),
    CONSTRAINT unique_doc_perms_role UNIQUE(document_id, role_id)
);
...
CREATE INDEX idx_doc_perms_role ON document_permissions(role_id);
CREATE INDEX idx_doc_perms_granted_by ON document_permissions(granted_by);
...
COMMENT ON CONSTRAINT check_doc_perms_target ON document_permissions IS 'Permission must target either a user OR a role, not both';
...
-- ============================================
-- 12. GRANT PERMISSIONS (Optional - adjust as needed)
-- ============================================
...
2. Check System Settings:
   SELECT setting_key, category FROM system_settings ORDER BY category, setting_key;
...
4. Begin Phase 1 API Development:
   ...
   - Implement /api/v1/auth/me
   - Implement password reset endpoints
   - Implement MFA endpoints

5. Start Phase 2 Database Work (in parallel):
   - Create OCR tables
   - Create Approval workflow tables
   - Create Document feature tables

ESTIMATED TIME TO IMPLEMENT PHASE 1 APIS: 40-60 hours
PARALLEL PHASE 2 DB WORK: Week 2-3
