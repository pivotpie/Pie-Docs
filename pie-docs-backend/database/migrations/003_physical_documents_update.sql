-- Physical Documents System Migration - UPDATE existing tables
-- Alters existing tables and creates missing ones
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='storage_locations' AND column_name='path') THEN
        ALTER TABLE storage_locations ADD COLUMN path TEXT DEFAULT '/';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='storage_locations' AND column_name='current_count') THEN
        ALTER TABLE storage_locations ADD COLUMN current_count INTEGER DEFAULT 0;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='storage_locations' AND column_name='utilization') THEN
        ALTER TABLE storage_locations ADD COLUMN utilization DECIMAL(5,2) DEFAULT 0.00;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='storage_locations' AND column_name='coordinates') THEN
        ALTER TABLE storage_locations ADD COLUMN coordinates JSONB;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='storage_locations' AND column_name='metadata') THEN
        ALTER TABLE storage_locations ADD COLUMN metadata JSONB DEFAULT '{}';
    END IF;
END $$;

-- Copy current_items to current_count if it exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='storage_locations' AND column_name='current_items') THEN
        UPDATE storage_locations SET current_count = COALESCE(current_items, 0);
    END IF;
END $$;
...
-- Now add FK constraint for barcode_records.asset_id
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'barcode_records_asset_id_fkey') THEN
        ALTER TABLE barcode_records ADD CONSTRAINT barcode_records_asset_id_fkey
        FOREIGN KEY (asset_id) REFERENCES physical_assets(id) ON DELETE SET NULL;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'physical_assets_barcode_id_fkey') THEN
        ALTER TABLE physical_assets ADD CONSTRAINT physical_assets_barcode_id_fkey
        FOREIGN KEY (barcode_id) REFERENCES barcode_records(id) ON DELETE SET NULL;
    END IF;
END $$;

-- ==========================================
-- Update Physical Documents Table
-- ==========================================
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='physical_documents' AND column_name='checked_out_by') THEN
        ALTER TABLE physical_documents ADD COLUMN checked_out_by UUID;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='physical_documents' AND column_name='checked_out_at') THEN
        ALTER TABLE physical_documents ADD COLUMN checked_out_at TIMESTAMP;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='physical_documents' AND column_name='due_back_at') THEN
        ALTER TABLE physical_documents ADD COLUMN due_back_at TIMESTAMP;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='physical_documents' AND column_name='notes') THEN
        ALTER TABLE physical_documents ADD COLUMN notes TEXT;
    END IF;
END $$;
...
-- Update print_jobs table if needed
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='print_jobs' AND column_name='template_id') THEN
        ALTER TABLE print_jobs ADD COLUMN template_id UUID;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='print_jobs' AND column_name='barcode_ids') THEN
        ALTER TABLE print_jobs ADD COLUMN barcode_ids UUID[] NOT NULL DEFAULT '{}';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='print_jobs' AND column_name='printer_id') THEN
        ALTER TABLE print_jobs ADD COLUMN printer_id UUID;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='print_jobs' AND column_name='copies') THEN
        ALTER TABLE print_jobs ADD COLUMN copies INTEGER DEFAULT 1;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='print_jobs' AND column_name='error') THEN
        ALTER TABLE print_jobs ADD COLUMN error TEXT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='print_jobs' AND column_name='completed_at') THEN
